...
class ClientData(context: Context) {
     ...
     * Pixel 4 XL: три перевыпуска ключа подряд, все три отвергнуты `tls: access denied` со
     ...
     * дня, и подсказка «подключитесь по Авто» переставала работать. Замер на Pixel 4 XL:
