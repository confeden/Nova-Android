...
object DiagnosticLogSanitizer {
     ...
     *
     * Правило намеренно узкое, и это размен, а не недосмотр.
     *
     * Метка времени и дата — это те же десять-тринадцать цифр с разделителями, и
     * слепое правило превращало их в `<phone>`, отнимая у журнала ровно то, по
     * чему события сопоставляют: строка tun2proxy приезжала как
     * `[<phone>:52:37 INFO tun2proxy]`, а «1757520123 188.114.97.3» — как
     * `handshake <phone>.114.97.***`. Широкое правило «10-15 цифр» ловит и метку
     * времени в секундах (десять цифр), и в миллисекундах (тринадцать), и дату
     * `2026-09-10 23` (те же десять), и список портов `80 443 853 1080`
     * (одиннадцать) — то есть стирает главные ключи сопоставления ради случая,
     * которого в этом журнале не бывает: поля телефона в приложении нет ни
     * одного, и попасть сюда номер может только текстом, который человек вставил
     * сам.
     *
     * Поэтому номером считается запись с явной приставкой страны — «+» или «00»
     * (так номер и вставляют) — либо российские одиннадцать цифр с 7 или 8.
     * Голый иностранный номер без приставки правило пропустит; цена ошибки в
     * другую сторону выше и наступает на каждой строке.
     */
    private fun looksLikePhone(value: String): Boolean {
        val bare = value.filter { it.isDigit() }
        // Международная запись: «+» или «00» вместо него. Длина — по E.164.
        if (value.startsWith("+")) return bare.length in 11..15
        if (bare.startsWith("00")) return bare.length in 12..17
        // Без приставки страны номером считается только российская запись:
        // одиннадцать цифр, начинающихся с 7 или 8.
