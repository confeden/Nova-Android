...
class FlowRadioGroup @JvmOverloads constructor(
     ...
     * Растр один потому, что ореол рисуется только у **отмеченной** кнопки, а
     * она одна. Прежде здесь стояло «все кнопки группы одной ширины
     * (`uniformItemWidth`), поэтому растр один» — с тех пор у строки входов Tor
     * ширина считается по тексту, и обоснование перестало быть верным, хотя
     * вывод остался. Промах кэша случается ровно на переносе отметки между
     * кнопками разной ширины, то есть раз на нажатие.
     */
