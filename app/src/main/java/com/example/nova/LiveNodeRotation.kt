...
class LiveNodeRotation(
    ...
    fun decide(
        ...
        if (rotations >= maxRotationsPerSession) {
            return Decision.Exhausted(
                if (maxRotationsPerSession == 0) {
                    "бесшовный переход выключен — он не вернул трафик ни разу из восьми замеров"
                } else {
    ...
    companion object {
         ...
         *
         * **Ноль.** Механизм измерен и не работает: на Mi A1 с искусственно
         * оборванным входящим потоком было восемь переходов за два прогона, и
         * **ни один не вернул трафик** — рукопожатие на новом узле проходило за
         * 57 мс, а данные не шли. Связь восстанавливало только обычное
         * переподключение, а каждый переход откладывал его на 22 секунды.
         *
         * Было четыре, потом два — «дешёвая проверка гипотезы „плохой узел“».
         * Проверка обошлась дорого: два перехода — это до 44 секунд мёртвого
         * туннеля перед тем, что действительно лечит, и снаружи ровно это и
         * читается как разрывы и потери пингов. Гипотеза проверена и не
         * подтвердилась восемь раз из восьми; платить за неё дальше нечем.
         *
         * Код перехода намеренно оставлен на месте: вернуть проверку — это
         * поднять число обратно, а не писать заново. Если однажды найдётся
         * случай, где смена узла лечит, он вернётся вместе с доказательством.
         */
        const val MAX_ROTATIONS_PER_SESSION = 0
