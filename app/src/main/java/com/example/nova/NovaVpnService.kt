...
class NovaVpnService : OperaNativeVpnService() {
    ...
    companion object {
         ...
         * процесс службы в `__cxa_finalize` — проверено на Pixel 4 XL, стоимость
         ...
         *
         * Замеры на Pixel 4 XL: живой узел отвечает за 320–550 мс, мёртвый выбирает
    ...
    private fun configureAndStartVpn(
        ...
        try {
            ...
            if (!warpAllowed) {
                ...
                // соединение принимается, туннель не открывается. Замер на Pixel 4 XL —
                ...
                if (diagnosticsMode) {
                    ...
                    // «НЕ ПОДКЛЮЧЕНО». Между тем отказ здесь непостоянный: на Pixel 4 XL
    ...
    private fun runMasquePhase(
        ...
        val masqueAttemptLimit = when {
            ...
            // здесь — «Останавливаем цикл» и экран «НЕ ПОДКЛЮЧЕНО». На Pixel 4 XL четырёх
        ...
        while (!isUserStopped && isConnectGenerationCurrent(connectGenerationId) && externalStopRequested?.invoke() != true) {
            ...
            val watchdogThread = Thread {
                try {
                    ...
                    // all devices/networks: Pixel 4A can report VALIDATED while rxBytes stay 0
            ...
            val engineThread = Thread {
                ...
                try {
                    if (transportMode.engine == "masque") {
                        ...
                        // не шлём.
                        //
                        // Замеры на Pixel 4 XL, 162.159.198.2:443: без пачки QUIC поднимался
                    ...
                    if (transportMode.engine == "masque") {
                        ...
                        // них означает ровно то, что мы пришли не туда. Замер на Pixel 4 XL:
                        ...
                        // ответила на запрос туннеля. Замер на Pixel 4 XL: ключ в таком
            ...
            if (transportMode.engine == "masque") {
                ...
                if (
                    ...
                    // поднимается не всегда. На Pixel 4 XL так и вышло: рабочий identity
    ...
    private fun ensureMasqueModeDiversity(
        ...
        //
        // Замер на Pixel 4 XL: узлы 162.159.198.x — та самая сеть из identity — принимают
        ...
        // В 1.10 упрощённый ordinary-wifi shortlist периодически уводил Pixel 4a в
     ...
     *
     * Замер на Pixel 4 XL, свежая identity, свой адрес 443: с `consumer-masque` сервер не
     ...
     * Pixel 4 XL: за два полных цикла в переборе не было ни одного `masque-v4`.
     */
    ...
    private fun hasTunnelConnectivity(
        ...
        // каждая проверка висела до собственного отказа резолвера. Замер на Pixel 4 XL:
